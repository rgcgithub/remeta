 ...
 * 
 *  Modified by Tyler Joseph - July 31 2023
 *  - added function to load genotypes as floats instead of doubles
 */
#ifndef PLINK2_PGEN_READER_H
#define PLINK2_PGEN_READER_H
...
public:
  ...
  void ReadFloatHardcalls(float* buf, size_t const& n, int const& thr, int variant_idx, int allele_idx);

  void Read(double* buf, size_t const& n, int const& thr, int variant_idx, int allele_idx);

  void ReadFloat(float* buf, size_t const& n, int const& thr, int variant_idx, int allele_idx);

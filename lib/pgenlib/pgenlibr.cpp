...
static const float kGenoRFloatPairs[32] ALIGNV16 = PAIR_TABLE16(0.0, 1.0, 2.0, -3.0);
...
void PgenReader::ReadFloatHardcalls(float* buf, size_t const& n, int const& thr, int variant_idx, int allele_idx) {
  if (!_info_ptr) {
    fprintf(stderr,"pgen is closed");
    exit(-1);
  }
  ...
  plink2::GenoarrLookup16x4bx2(_pgv[thr]->genovec, kGenoRFloatPairs, _subset_size[thr], buf);
}
...
void PgenReader::ReadFloat(float* buf, size_t const& n, int const& thr, int variant_idx, int allele_idx) {
  if (!_info_ptr) {
    fprintf(stderr,"pgen is closed");
    exit(-1);
  }
  ...
  plink2::Dosage16ToFloatsMinus9(_pgv[thr]->genovec, _pgv[thr]->dosage_present, _pgv[thr]->dosage_main, _subset_size[thr], dosage_ct, buf);
}

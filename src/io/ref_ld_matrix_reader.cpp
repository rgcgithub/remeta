...
void RefLDMatrixReader::load_gene_ld_mat(gene_ld_mat_t& gene_ld,
  ...
  for (size_t i = 0; i < variant_ids.size(); ++i) {
    gene_ld.gene_variant_idx_in_file[variant_ids[i]] = (int32_t)i;
    gene_ld.idx_in_file_gene_variant[(int32_t)i] = variant_ids[i];
  }
  ...
  for (size_t i = 0; i < gene_variants.size(); ++i) {
    ...
    gene_ld.gene_variant_idx_in_G[gene_variants[i]] = 
      (int32_t)gene_ld.gene_variant_idx_in_G.size();
  }
...
void RefLDMatrixReader::load_gene_ld_mat(Eigen::MatrixXf& G,
  ...
  unordered_map<string, int32_t> gene_var_to_idx;
  unordered_map<int32_t, string> idx_to_gene_var;
  unordered_map<string, int32_t> var_to_G_idx;
